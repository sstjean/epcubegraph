...
#
# Traffic flow:
#   Container Apps → (VNet) → Private Endpoint → Key Vault
#   Container Apps → (VNet) → delegated subnet → PostgreSQL Flexible Server
#   Deployer (CD runner) → (public internet) → ip_rules whitelist → Key Vault
