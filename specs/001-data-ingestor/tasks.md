...
**Tests**: Included — constitution mandates TDD with 100% code coverage (non-negotiable).\n\n> **⚠️ Migration Note**: These tasks were implemented using VictoriaMetrics as the storage backend. VictoriaMetrics is being removed from the project and will be replaced with Azure SQL Database (serverless). Task descriptions below reflect the original implementation and are retained as historical reference. Code-level references (file names, config keys) still exist in the codebase pending the migration.
...
**Goal**: Deploy epcube-exporter as a Container App in Azure, scraped directly by the data store. EP Cube cloud credentials stored in Key Vault.

**Independent Test**: Run `infra/deploy.sh` to verify epcube-exporter Container App deploys, the data store scrapes it, and metrics appear.

**FRs covered**: FR-015, FR-016, FR-017

**Also covers (indirectly)**: FR-001, FR-002 (epcube-exporter polls both EP Cube 1.0 and 2.0 devices via the cloud API), FR-006 (epcube-exporter retries on next poll cycle; data store handles scrape failures gracefully)
...
**Checkpoint**: `terraform validate` passes. epcube-exporter Container App deploys with the data store scraping it directly.

---

## Phase 4: User Story 1 — Ingest Telemetry / Azure Infrastructure (Priority: P1)

**Goal**: Deploy VictoriaMetrics on Azure Container Apps with internal-only ingress, promscrape scraping epcube-exporter, Key Vault for secrets, indefinite retention, deduplication
...
**Note on FR-011 (UTC normalization)**: The data store stores all timestamps as Unix epoch (inherently UTC). No application-level normalization code is required.
...
**Checkpoint**: `terraform validate` passes. Data store accepts ingestion from epcube-exporter.

---

## Phase 5: User Story 2 — Expose Telemetry via Versioned API (Priority: P2) 🎯 MVP

**Goal**: C# ASP.NET Core Minimal API querying the data store, Entra ID JWT auth, derived grid calculation, versioned at /api/v1

**Independent Test**: Issue authenticated API requests for /query_range, /devices, /grid with test data in the data store. Verify correct results. Verify unauthenticated requests return 401. Verify empty time ranges return empty result (not error).
...
- [x] T027 [P] [US2] Implement HealthEndpoints: GET /api/v1/health (unauthenticated, static HealthResponse("healthy", "ok") with 200 — no data store dependency) in api/src/EpCubeGraph.Api/Endpoints/HealthEndpoints.cs
- [x] T028 [US2] Implement QueryEndpoints: GET /api/v1/query, GET /api/v1/query_range, GET /api/v1/series, GET /api/v1/labels, GET /api/v1/label/{name}/values as authenticated query passthrough to the data store in api/src/EpCubeGraph.Api/Endpoints/QueryEndpoints.cs
...
- [x] T042 [US2] Configure structured JSON logging: builder.Logging.AddJsonConsole(), add ILogger injection and log statements for auth failures (401/403), data store query errors, and request durations (FR-020) in api/src/EpCubeGraph.Api/Program.cs
...
- [x] T036 Run quickstart.md end-to-end validation: clone, dotnet build, dotnet test, docker compose build, az deployment validate. Also verify SC-001/SC-002 infrastructure: data store uses persistent storage, scrape metrics are populated after deployment, and ephemeral storage edge case (L2) is mitigated by persistent volume mount
- [x] T037 [P] Security review: verify all telemetry endpoints reject unauthenticated requests (SC-004), verify /health exposes no telemetry data, verify data store is internal-only (no external ingress)
...
- **US2 (Phase 5)**: Depends on Foundational — uses data store client interface and auth from Phase 2
...
- **US2 (P2)**: Can start after Foundational — no dependencies on US1 or US3 (uses Testcontainers for integration tests, not Azure deployment)
...
3. US1 → Terraform deploys → data store accepts ingestion and scrapes epcube-exporter
