...
- **FR-017**: Container Apps MUST be configured to auto-restart on failure.
- **FR-018**: The deployment MUST NOT require any manual installation steps beyond running `infra/deploy.sh` with a configured `terraform.tfvars`; all dependencies MUST be resolved within the container images and Terraform configuration.
...
- The remote-write bearer token is provisioned in Azure Key Vault and validated by vmauth for any external remote-write sources.
