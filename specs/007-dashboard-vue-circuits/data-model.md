...
|-----|-----------|---------|-------|
| `vue_device_mapping` | JSON string | `{}` | Maps EP Cube device IDs to Vue panels with alias: `{"epcube3483": [{"gid": 480380, "alias": "Main Panel"}]}` |
...
- **Format**: JSON object — keys are EP Cube device IDs (strings), values are arrays of panel objects with `gid` (number) and `alias` (string — panel display name for Circuits page)
- **Example**: `{"epcube3483": [{"gid": 480380, "alias": "Main Panel"}, {"gid": 480544, "alias": "Subpanel 1"}], "epcube5488": [{"gid": 480577, "alias": "Subpanel 2"}]}`
