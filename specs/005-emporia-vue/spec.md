...
- Q: Should the exporter request kWh and convert, or request Watts directly? → A: Request in Watts directly from the Emporia API. No manual conversion needed.
...
- **FR-003**: The exporter MUST retrieve per-circuit power readings for every channel on every configured Vue device, including the panel mains (total power in/out). The exporter MUST request data in Watts directly from the Emporia API (unit: `Watts`) to avoid manual kWh-to-watts conversion and floating-point rounding artifacts.
