...
- Q: Should replacement detection match removals+additions across consecutive discovery cycles or same cycle only? → A: Both. The exporter first looks for same-cycle add+remove pairs and, if none match, checks for cross-cycle pairs by finding the most recent `status='removed'` device whose alias matches a newly-added device. The Settings page also exposes a manual merge for ad-hoc cases. (Updated after PR #137 validated this end-to-end: a real hardware swap predating the feature deploy was only detectable cross-cycle.)
...
- **FR-010**: When the exporter detects an alias match between a newly-added device and a recently-removed device, the system MUST record a pending replacement prompt for the user. Matching considers both same-cycle pairs (one removal + one addition with the same alias in the same discovery cycle) and cross-cycle pairs (a newly-added device's alias matches the most recently removed device with that same alias). The Settings page also exposes a manual merge UI for ad-hoc cases that bypass the heuristic.
