...
- [x] T015 [P] Implement dashboard/src/utils/formatting.ts: formatWatts(watts: number) → string (auto-scale W/kW/MW with 1 decimal), formatPercent(value: number) → string, formatTimestamp(epoch: number) → string (locale-aware), formatRelativeTime(epoch: number) → string ("5m ago"), null/NaN guards returning "—"
...
- [x] T024 [US1] Implement CurrentReadings component in dashboard/src/components/CurrentReadings.tsx: renders as <section> with <h2> heading (FR-015), fetches device list via fetchDevices(), issues 5 instant queries for epcube_battery_state_of_capacity_percent, epcube_battery_power_watts, epcube_solar_instantaneous_generation_watts, epcube_grid_power_watts, epcube_home_load_power_watts, groups battery+solar devices by alias into DeviceGroups (see plan.md Design Decisions), renders DeviceCard per group, sets up polling via createPollingInterval (30s default per FR-012), shows loading skeleton with aria-busy="true" on initial fetch (FR-015), wraps in ErrorBoundary, displays lastRefreshed timestamp via formatRelativeTime
